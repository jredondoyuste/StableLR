...
id = "/groups/astro/jredondo/2D_NLKG/data/Convergence/2d_nlkg_N_$(Nmodes)_l_$(l0_1)_$(l0_2)_A_$(round(A_tot,sigdigits=2))"
#############################

S = Legendre(-1..1)
sqrtfr=0.27683
freqs = [sqrt(l*(l+1)) for l in 0:Nmodes]
Γk = [2.0*exp(-0.229458 - 2.14058*l)/sqrtfr for l in 0:Nmodes]  # WKB approximation to damping times in high l limit
...
function rhs!(du, u)
    ...
    du[1] = u[2]
    nlterm = nl(phi)
    du[2] = @. - freqs^2 * phi + nlterm - Γk * psi
end 
